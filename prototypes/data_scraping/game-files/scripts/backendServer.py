...
@route('/api/dotrgb')
def dotrgb():
    """api to return the current dot color, variable update rates"""
    rgb = dot.curDotRGB
    return {'r': rgb[0], 'g': rgb[1], 'b': rgb[2]}

@route('/api/mainweather')
def mainWeather():
    """api to return the full weather data for a random city"""
    return weather.getRandMainWeather()

@route('/api/mainweather/temp')
def mainTemp():
    """api to return just the tempurature of a random city"""
...
def mainPres():
    """api to return just the pressure of a random city"""
    main = weather.getRandMainWeather()
    city = list(main.keys())[0]
...
def mainHumid():
    """api to return just the humidity of a random city"""
    main = weather.getRandMainWeather()
    city = list(main.keys())[0]
...
def windWeather():
    """api to return the wind information of a random city"""
    return weather.getRandWindWeather()

@route('/api/windweather/speed')
def windweather():
    """api to return the wind speed of a random city"""
...
def windDir():
    """ api to return the wind direction of a random city"""
    wind = weather.getRandWindWeather()
    city = list(main.keys())[0]
